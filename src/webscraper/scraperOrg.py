...
class WebScraperOrg:
    ...
    async def crawlOrg(self, url=None, visited=None):
        """
        ...
        if url == self.rootUrl:
            self.rootPageText = text
            self.logger.info(f"Saving Root Page Text, Length: {len(self.rootPageText)}")
        ...
        for word in words:
            if "@" in word:
                if word not in self.emails:
                    self.logger.info(f"Found Email: {word}")
        ...
        for link in soup.find_all('a', href=True):
            ...
            if full_url.startswith(self.rootUrl) and full_url not in visited:
                await self.crawlOrg(full_url, visited)
    ...
    async def close(self):
        ...
        self.logger.info(f"Root Page Text Length: {len(self.rootPageText)}")